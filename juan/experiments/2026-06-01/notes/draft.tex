...
$$
In other words, it maps a $k$-chain $e_{i_0...i_k}$ to the alternating linear combinations of the $(k-1)$-chains associated to the  ``boundary'' of the simplex $i_0...i_k$.
...
|f|^2 = \sum_{ij} |f_{ij}|^2,
$$
i.e. these are the standard euclidean norms.

\section{Description of the data obtained from the experiments}

In the folder:

\quad \verb+ranking-futbol/juan/experiments/2026-06-01/txt/+

we provide text files such as:
\begin{itemize}
\item 
\verb+LIGA-METRIC-hodge-decomposition-nodes.txt+
\item 
\verb+LIGA-METRIC-hodge-decomposition-links.txt+
\item 
\verb+LIGA-METRIC-hodge-decomposition-triangles.txt+
\item 
\verb+LIGA-METRIC-hodge-decomposition-globals.txt+
\end{itemize}
where \verb+LIGA+ and \verb+METRIC+ represent any of the leagues and metrics.

\begin{itemize}

\item The first file:

\quad \verb+LIGA-METRIC-hodge-decomposition-nodes.txt+

\noindent contains the columns:

\quad \verb+i w[i]=w_i r[i]=r_i tr[i]=tr_i+

\noindent denoting the label or number associated to each node $i$ (which it is also the true ranking), the corresponding inferred rating $w_i$, the corresponding inferred ranking $r_i$ and the corresponding true rating \verb+tr_i+ from the final position in the table (not from the metrics).

\item The second file:

\quad \verb+LIGA-METRIC-hodge-decomposition-links.txt+

\noindent contains the columns:

\quad \verb+l i j f[l]=f_ij g[l]=g_ij s[l]=s_ij h[l]=h_ij+

\noindent denoting the label or number $l$ associated to each link, the corresponding nodes $i$ and $j$, and the corresponding components of $f$, $g$, $s$ and $h$ on such link.

\item The third file:

\quad \verb+LIGA-METRIC-hodge-decomposition-triangles.txt+

\noindent contains the columns:

\quad \verb+t i j k u[t]=u_ijk+

\noindent denoting the label or number $t$ associated to each triangle, the corresponding nodes $i$, $j$ and $k$, and the corresponding components of the $u$.

\item The fourth file:

\quad \verb+LIGA-METRIC-hodge-decomposition-globals.txt+

\noindent contains the columns:

\quad \verb+|f| |g| |s| |h| |u|+

\noindent denoting the norm of the corresponding vectors in $C^*$.
These are global characteristics.
\end{itemize}
