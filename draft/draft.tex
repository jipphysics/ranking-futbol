...
Namely, the rank-based 1-cochain $f_{ij}=\frac{1}{2}(i-j)$ and the tournament point 1-cochain $f_{ij}=\frac{1}{2}(\pi_j-\pi_i)$.
Note, $f_{ij}=-f_{ji}$ holds by definition.
These 1-cochains $f$ define a fully connected network or graph, since all pairs of teams play two matches against each other in each league.
Each of these fully connected graphs of $N=n+1$ nodes is promoted to a simplicial complex compose of 0-simplices labeled by $i$, 1-simplices labeled by $ij$ and 2-simplices labeled by $ijk$ where $i,j,k$ are different elements in $\{0,1,2,...,n\}$.
