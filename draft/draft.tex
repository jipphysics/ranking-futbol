...
\end{enumerate}
Moreover, the Singular Value Decomposition of previous emprically determined metrics are also considered in order define the so-called {\em styles}~\cite{chacoma2025data}.
...
Correspondingly, the coefficients $f_{ij}$ are promoted to be the components of a 1-cochain  $f$, where the anti-symmetric relation $f_{ji}=-f_{ij}$ is imposed by definition.
