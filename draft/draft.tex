...
Moreover, in order to test the inference of ratings, two types of ground-truth metrics are also defined.
Namely, the rank-based metric $m_{ij}=\frac{1}{2}(i-j)$ and the tournament point metric $m_{ij}=\frac{1}{2}(\sigma_j-\sigma_i)$.
Notice, $m_{ij}$ is generally independent of the metric $m_{ji}$ since they belong to different matches between $i$ and $j$.
In the case of the ground-truth metrics, it holds that $m_{ij}=-m_{ji}$.
