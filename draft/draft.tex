...
Suppose that for each distinct pair of teams $i$ and $j$, a pairwise comparison score $\psi_{ij}=-\psi_{ji}$ is given.
Informally, $\psi_{ij}>0$ suggests that team $i$ should be rated higher than team $j$, while $\psi_{ij}<0$ suggests the opposite.
...
$$
\sum_{i<j} \bigl(\psi_{ij}-(w_i-w_j)\bigr)^2.
$$
