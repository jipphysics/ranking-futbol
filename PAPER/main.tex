...
It can be easily shown that $d$ admits the decomposition $d=d_0+d_1+...+d_n$ with $d_k(C^*)=d_k(C_k^*)\subseteq C_{k+1}^*$ and, also, $d^2=0$ easily follows from its definition and the fact that $\partial^2=0$.
...
Correspondingly, $f^{\sharp}$ denotes the chain univocally associated to the cochain $f\in C^*$ and $f\cdot f' = f^{\sharp}\cdot f'^{\sharp}$ is the inner product over $C^*$ induced by the inner product over $C$.
Notice that $(c^{\flat})^{\sharp} = c$ and $(f^{\sharp})^{\flat}=f$.
