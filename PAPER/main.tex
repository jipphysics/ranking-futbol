...
%
Podemos modelar este parametro a partir de una distribución de Dirichlet,
%
\begin{align*}
...
Lo hicimos a partir de un procedimiento tipo boostraping para $1000$ muestreos con remplazo, utilizando los datos de la liga Inglesa, Francesa, Italiana y Epañola.
En la Fig. \ref{fi:stats} (a), mostramos la cumulative distribution function (CDF) del true rating junto con el ajuste. El grafico incluye también una densidad gaussiana de valor medio y desviacion estandar igual a los valores muestrales $\bar{R}_T=52.45$ $(s_{R_T}=18.56)$, a modo de referencia.
Los valores obtenidos del ajuste para los parametros son 
$\alpha_g= 0.85\pm0.03$, $\alpha_e=2.60\pm0.07$, y $\alpha_p=0.46\pm0.05$.
Respecto a los resultados obtenidos, en primer lugar, podemos observar que el Multinomial-Dirichlet (M-D) model ajusta bien los datos empiricos en toda la curva, notandose un destacable rendimiento en la zona de raiting bajos. Comaprando estos resultados con la curva gaussiana, vemos que los true rating muestran colas mas livianas y un exceso de concentración en la zona de la mediana. En la Fig. \ref{fi:stats} (b), la realción entre los quantiles resalta el buen renidmiento del modelo y las diferencias claras repecto del modelo gaussiano en la zona de las colas y de la mediana.  
...
Nuestra idea es comparar el true rating con el rating obtenido a partir del metodo de HodgeRank. Naturalemente, estas cantidades estan en escalas distintas, por lo tanto para hacer la comparación es necesario llevar los valores de ambas variables a la misma escala. Por tal motivo, decidimos estandarizar las variables y hacer la comparacion sobre el {\it z-score} de cada una de estas.
%
...
%
%
En la Fig. \ref{fi:rating} (d) mostramos la CDF de las diferencias $\Delta R$ observadas en cada liga. En cada una de estas hemos agrupado los datos provenientes de las 10 metricas. A modo de referencia, el grafico incluye tambien la CDF de una densidad gaussiana de valor medio y desvio estandar igual al del conjunto formado por el total de los datos.
Note que dependiendo de cada liga la CDF se aparta de manera distinto de la referencia gaussiana, mostrando diferentes tipo de comportamiento. Podemos ver que las ligas Inglesa e Italiana presentan colas mas livianas que la gaussiana, indicando una densidad de probabilidad aguda alrededor de la media. Por el conrario, los datos asociado a liga española muestran colas levemente pesadas, mostrando la presencia de casos mas extremos. Por otro lado, los datos asociados a las ligas Francesa y alemana parecen comportarse de manera normal.
Estas observaciones parecen indicar que hay ligas donde el metric rating tiende a diferenciarse mas del true rating. En lo que sigue cuantificamos estas diferencias en funcion de las distintas metricas.

Nuestro objetivo es cuantificar la diferencia entre la curva de true rating, $R_T$, y las curvas de metric rating, $R_M$, asociada a una metrica $m$ en una liga $l$. Para esto, definimos la distancia $D$, como  el Root Mean Square Error (RMSE), el valor medio de las diferencias cuadraticas entre dos curvas. Ademas, calculamos el coeficiente de Person entre ambas curvas, para estudiar el comportamiento de la tendencia.
