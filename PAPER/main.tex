...
Within this framework, we define a ball possession interval (BPI) as the set given by a continuous sequence of events {\color{red} [Yo pondría ``given by a sequence of consecutive events'']]} generated by a single team. Note that each BPI contains information from only one team.  
...
\begin{enumerate}
    ...
    \item {\it Midfield play.} If within a BPI it is observed that $\bar{x}\leq60$ and $\bar{x}\geq40$ {\color{red}[Yo pondría al revés, i.e. ``$\bar{x}\geq40$ and $\bar{x}\leq60$'']}, i.e., the possession develops predominantly in the central area of the pitch, the total possession time is reported; otherwise, a value of $0$ is reported. The purpose of this variable is to measure the time the team spends in the midfield zone.
...
{\color{red} [Me resulta raro el t\'ermino ``complex networks'' teniendo en cuenta que son redes totalmente conectadas.]}

In what follows, we present our proposal to represent performance metrics in terms of complex networks.  
We define $M(i,j,g)$ as the performance metric corresponding to team $i$ when facing team $j$ in match $g$. For example, this quantity may represent the number of shots on goal taken by FC Barcelona {\color{red} [Agregaría ``represented by $i$'']} when playing against Real Madrid {\color{red} [Agregaría ``represented by $j$'']} in the first encounter {\color{red} [Yo agregaría ``$g$'']} of the Spanish league {\it La Liga}.  
%
...
$$
{\color{red} [La sumatoria sobre $g_1$ y $g_2$ es una notación rara, ya que la expresi\'on dentro de la sumatoria depende de $g$, pero no de $g_1$ y $g_2$.]}
%
...
$$
%
{\color{red} [La siguiente oraci\'on es un poco redundante?]}
...
To infer ratings from a given metric, we seek to minimize the sum
$$
\chi(w)
=
\sum_{ij}
...
|f_{ij}-(w_j-w_i)|^2
$$
with respect to the vector $w$ of node weights $w_i$.
...
From a physical perspective, the ratings $w_i$ can be interpreted as potentials defined on the nodes, while $f_{ij}$ plays the role of forces acting along the edges. 
Under this interpretation, minimizing $\chi$ corresponds to finding the potentials $w_i$ whose induced differences $w_j-w_i$ reproduce the forces $f_{ij}$ as closely as possible in the least-squares sense.

To address this issue and efficiently infer a solution, Jiang et al.~\cite{jiang2011statistical} introduced HodgeRank, which builds on the Hodge decomposition of the edge flow  $f_{ij}$s using discrete calculus on Higher Order Networks~\cite{grady2010discrete,lim2020hodge,bianconi2021higher}. In this framework, the pairwise comparison values $f_{ij}$ are interpreted as a flow on the edges of the comparison network and is decomposed into gradient, curl, and harmonic components. The ratings correspond to the gradient component of this decomposition and can be obtained by solving a pair of singular linear systems~\cite{perotti2025analysis}.

%An hypergraph is a subset $K$ of the powerset of the set $\{0,1,...,n\}$ of $n++1$ nodes.
%An hypergraph is the most general representation of a Higher Order Network discrete topological structure.
%A subset of an hypergraph $K$ is called an hyperlink.
%
%An abstract simplicial complex $K$ is a particular type of hypergraph that satisfy the property of topological closure: if $s$ is an hyperlink of $K$ and $r$ is a nonempty subset of $s$, then $r$ is also an hyperlink of $K$.
%The hyperlinks of a simplicial complex are called simplices.
%In particular, a simplex containing $k$ nodes is called a $k$-simplex.
%The set of $k$-simplices of a simplicial complex $K$ is denoted by $K_k$.
%
%To each simplex $s$ we associate an abstract vector $e_s$, so we can define the vector space $C$ spanned by the canonical basis $\{e_s:s\in K\}$.
%The vectors in $C$ are called chains.
%In particular, $e_s$ is a $k$-chain if $s$ is a $k$-simplex.
%The space of chains can be decomposed into the direct sum $C = C_0\oplus C_1\oplus ...\oplus C_n$, where $C_k$ is the subspace of $C$ generated by the subset $\{e_s:s\in K_k\}$ of basis vectors associated to the $k$-chains in the simplicial complex.
%The additional convenient definition $C_{-1} = C_{n+1} = \{0\}$ is introduced in terms of the zero vector of $C$.
%
%A notion of orientation can be defined on the space of chains $C$.
%Namely, after adopting an arbitrary enumeration of the nodes in $K$, we define the positive orientation $e_{j_0j_1...j_k} := e_s$ if $s=\{i_0,i_1,...,i_k\}$ and $i_0<i_1<...<i_k$, the negative orientation $e_{j_0j_1...j_k} := -e_s$ if $j_0j_1...j_k$ is an odd permutation, and $e_{j_0j_1...j_k}=0$ otherwise.
%In this way, we can define the boundary linear operator $\partial$ over $C$ by how it acts on the canonical basis
%$$
%\partial e_{i_0...i_k} = \sum_{\ell=0}^k
%(-1)^{\ell} e_{i_0...i_{\ell-1}i_{\ell+1}...e_k}.
%$$
%It is not difficult to show that $\partial = \partial_0 + \partial_1 + ... + \partial_n$ with $\partial_k(C) = \partial(C_k) \subseteq C_{k-1}$ meaning that the boundary operator maps a simplicial complex to its boundary and, thanks to the alternating nature of the definition, that $\partial^2 = 0$ meaning that the boundary of a boundary has no boundary.
%
%The dual space $C^*$ of $C$ is called the space of cochains.
%The dual basis is defined in terms of the dual vectors $e^s$ in $C^*$, so $e^s(e_r) = \delta^s{}_r$ by definition.
%The space of cochains also admits a direct sum decomposition $C^* = C_0^*\oplus ... \oplus C_n^*$ where $C_k^*$ is the dual space associated to $C_k$ and the covectors in $C_k^*$ are called $k$-cochains.
%
%The coboundary operator $d:C^*\to C^*$ is defined as the the dual adjoint of the boundary operator, i.e. $df(c):=f(\partial c)$ for any cochain $f\in C^*$ and any chain $c\in C$.
%This is the discrete analogous of the generalized Stoke's theorem and $d$ is the discrete analogous of the differential operator.
%It can be easily shown that $d$ admits the decomposition $d=d_0+d_1+...+d_n$ with $d_k(C^*)=d(C_k^*)\subseteq C_{k+1}^*$ and, also, $d^2=0$ easily follows from its definition and the fact that $\partial^2=0$.
%Notice how the boundary operator $\partial$ decreases $k$ while the coboundary operator $d$ increases $k$.
%
%Since $C$ is of finite dimension $|K|$, a non-singular inner product $c\cdot c'$ over $C$ establishes an (antilinear) isomorphism between $C$ and $C^*$ by the Rietz theorem, where $c^{\flat}\in C^*$ denotes the cochain univocally associated to the chain $c$ such that $c^{\flat}(c') = c\cdot c'$ for every $c'\in C$.
%Its inverse is denoted by $f^{\sharp}$ for every $f\in C^*$, $(c^{\flat})^{\sharp} = c$ and $(f^{\sharp})^{\flat}=f$.
%Notice that $f\cdot f' := f^{\sharp}\cdot f'^{\sharp}$ is the inner product over $C^*$ naturally induced by the inner product over $C$.
%These inner product equips the combinatorial topological space $K$ with some geometric notions.
%
%The dual coboundary operator $d^*$ is a linear operator over $C^*$ defined as the inner product adjoint of $d$, i.e. $d^*f.g = f.dg$ for every cochains $f$ and $g$ in $C^*$.
%It can be decomposed as $d^* = d_0^* + d_1^* + ... + d_n^*$ where $d_k^*(C^*) = d^*(C_k^*) \subseteq C_{k+1}^*$, so $d^*$ raises $k$.
%Additionally, $(d^*)^2=0$ easily follows from $d^2=0$.
%
%The Hodge Laplacian is defined as $L:=(d^*+d)^2 = d^*d+dd^*$.
%It is self adjoint, so $L^*=L$, and it has a non-negative real spectrum, with at least one zero eigenvalue. 
%It can be decomposed as $L=L_0\oplus L_1\oplus ... \oplus L_n$ with $L_k = L_k^{\uparrow} \oplus L_k^{\downarrow}$ where $L_k^{\uparrow} = d_k^*d_k$ and $L_k^{\downarrow}=d_{k-1}d_{k-1}^*$ are called the $k$-th up- and low- Hodge Laplacians.
%Moreover, $L_k(C^*) = L(C_k^*) \subseteq C_k^*$.
%
%Based on previous constructions, Hodge proves the following decomposition
%$$
%C_k^* = \mathrm{rng}\, d_k^* \oplus \mathrm{krn}\, L_k \oplus \mathrm{rng}\, d_{k-1}
%$$
%where $\mathrm{rng}\, d_k^*$ is the range of $d_k^*$ and $\mathrm{krn}\, L_k$ is the kernel of $L_k$.
%As a consequence, any $k$-cochain $f\in C_k^*$ can be written as $f=s+h+g$ for a corresponding unique coexact or solenoidal component $s\in \mathrm{rng}\, d_k^*$, a corresponding harmonic component $h\in \mathrm{krn}\, L_k$ and a corresponding exact or gradient component $g\in \mathrm{rng}\, d_{k-1}$.
