...
La idea de esta sección es definir un modelo estadistico para el rating real, $R_T$, el cual definimos como la cantidad de puntos totales obtenidos durante la liga.
%
Note que esta es una variable estocastica que depende de la cantidad de partidos ganados, empatados y perdidos.
%
Ademas, es sabido que en las ligas de futbol, un equipo obtiene 3 puntos cuando gana, 1 cuando empata y 0 cuanto pierde. 
%
En este marco, definimos las variables estocasticas $G_i$, $E_i$ y $P_i$, como la cantidad de partidos ganados, empatados y perdidos por el equipo $i$. Luego, el rating real de este puede escribirse como,

$$
{R_T}_i = 3G_i + 1E_i.
$$

Definiendo ${\bf p}_i = ({p_g}_i, {p_e}_i, {p_p}_i)$ como el vector que contiene las probabilidades de ganar, empatar o perder un partido, para el equipo $i$, podemos modelar la cantidad de partidos ganados, empatados y perdidos por ese equipo con una distribucion multinomial,

$$
(G_i,E_i,P_i) \sim Multinomial( n, {\bf p}_i ),
$$
%
Donde $n$ es la cantidad total de partidos jugados, lo cual en una liga es igual para todos los equipos. 
Note que cada equipo tendrá asociado un vector ${\bf p}$ diferente, dependiendo de su nivel competitivo. 
%
Podemos modelar este vector a partir de una distribución de Dirichlet,
...
\end{align*}
%
Esta distribución, en función del conjunto de parámetros $\boldsymbol{\alpha}$, genera vectores de tres componentes, ${\bf p}_i$, que cumplen con las propiedades requeridas para una terna de probabilidades:  
...
En este marco, la probabilidad de que un equipo durante la liga gane $G=g$ partidos y empate $E=e$, viene dada por,
...
\end{equation}
%
En esta expresion, las densidades de probabilidad son,
%
\begin{align*} 
f_M( g,e ; n | {\bf p} ) &= 
...
\end{align*}
%
la densidad multinomial condidicionada y la densidad de Dirichlet, respectivamente. Ademas, el espacio de integración, $S$, es el simplex generado por las eqs. \ref{simplex}.
%
...
A partir de la eq. \ref{proba}, se puede obtener la distribucion de probabilidad teorica $R_T$. Los grados de libertad que otorgan los parametros $\boldsymbol{\alpha}$, permiten ajustando la curva teorica a los datos. 
En la Fig. \ref{fi:stats}
